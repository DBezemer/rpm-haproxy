...
run-docker: build-docker
	...
	docker run --volume $(HOME)/RPMS:/RPMS --rm -e WITH_PROMETHEUS=${USE_PROMETHEUS} -e RELEASE=${RELEASE} haproxy-rpm-builder7:${VERSION}-${RELEASE}
	docker run --volume $(HOME)/RPMS:/RPMS --rm -e WITH_PROMETHEUS=${USE_PROMETHEUS} -e RELEASE=${RELEASE} haproxy-rpm-builder8:${VERSION}-${RELEASE}
	docker run --volume $(HOME)/RPMS:/RPMS --rm -e WITH_PROMETHEUS=${USE_PROMETHEUS} -e RELEASE=${RELEASE} haproxy-rpm-builder-amzn2:${VERSION}-${RELEASE}
